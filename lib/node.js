  ...
  startDaemon (done) {
    parseConfig(this.path, (err, conf) => {
      ...
      this.subprocess = run(this.exec, ['daemon'], {env: this.env})
        ...
        .on('data', data => {
          console.log(11, (data + ''))
          var match = (data + '').trim().match(/Gateway \(readonly\) server listening on \/ip4\/(.+?)\/tcp\/(\d*)/)
          if (match) {
            this.gatewayIp = match[1]
            this.gatewayPort = match[2]
            console.log(111, this.gatewayIp, this.gatewayPort)
          }

          match = (data + '').trim().match(/API server listening on (.*)/)
          if (match) {
